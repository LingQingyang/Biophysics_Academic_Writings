...
def temperature_scale(logits, model, new_past, config, temperature, temperature_horizon):
    ...
    # EXTRA CREDIT ONLY
        ...
        for t in range(config.vocab_size):
            ...
            second_logits = top_k_logits(second_logits, config.top_k)
        ...
        first_logits = None
        ### START CODE HERE ###
        scaled_joint_logits = joint_logits / temperature
        first_logits = torch.logsumexp(scaled_joint_logits, dim=1)
        ### END CODE HERE ###
