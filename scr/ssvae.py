...
class SSVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        rec_all = -ut.log_bernoulli_with_logits(x, logits)
        kl_z_all = ut.kl_normal(m, v, self.z_prior_m, self.z_prior_v)
        
        rec_reshaped = rec_all.view(self.y_dim, batch_size)
        kl_z_reshaped = kl_z_all.view(self.y_dim, batch_size)
        
        y_prob_T = y_prob.t()
        
        rec = (y_prob_T * rec_reshaped).sum(dim=0)
        kl_z = (y_prob_T * kl_z_reshaped).sum(dim=0)
        
        y_prior_logprob = torch.log(torch.ones_like(y_prob) / self.y_dim)
        kl_y = (y_prob * (y_logprob - y_prior_logprob)).sum(dim=1)
        
        nelbo = kl_y + kl_z + rec
        
        return nelbo.mean(), kl_z.mean(), kl_y.mean(), rec.mean()
        ### END CODE HERE ###
        ################################################################################
