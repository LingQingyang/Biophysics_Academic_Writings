...
def log_likelihood(model, text):
    ...
    with torch.no_grad():
        ...
        ### START CODE HERE ###
        logits, _ = model(text)  # [1, T, vocab_size]

        # ignore first token
        inputs = logits[:, :-1, :].reshape(-1, logits.size(-1))  # [T-1, vocab_size]
        targets = text[:, 1:].reshape(-1)                        # [T-1]

        loss_fn = nn.CrossEntropyLoss(reduction='sum')
        neg_log_likelihood = loss_fn(inputs, targets)
